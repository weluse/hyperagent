...
Resource.prototype.link = function link(rel, params) {
  // Ahhh, how do I construct it here?!
};
...
LazyResource.prototype._makeGetter = function _makeGetter(object) {
  ...
  return function () {
      return new options.factory(object, _.clone(parent._options));
  }.bind(this);
};
...
function LinkResource(object, options, params) {
  // Inherit from Resource
  Resource.call(this, options);
  ...
  if (object.templated) {
    if (!params) {
      console.warn('Creating a templated link without params. ' +
          'Use Resource.link() instead!');
    } else {
      href = (new URI.expand(href, params)).toString();
    }
  }

  // The href is OPTIONAL, even for links.
  if (href) {
    this._navigateUrl(href);
  } else {
