...
Resource.prototype._resolveUrl = function _resolveUrl(value) {
  if (!value) {
  ...
  if (uri.is('absolute')) {
    ...
    return (new URI(this._options.url))
      .resource(value).normalize().toString();
  } else {
    return new URI([this._options.url, value].join('/'))
      .normalize().toString();
  }
};
...
Resource.prototype._navigateUrl = function _navigateUrl(value) {
  this._options.url = this._resolveUrl(value);
};
